...
class UserController extends Controller
{
    ...
    public function update(User $user, Request $request): JsonResponse
    {
        ...
        try {
            ...
            $profile->first_name = $request->has('first_name') && !is_null($request['first_name']) && $request['first_name'] != '' ? $request['first_name'] : $profile->first_name;
            $profile->last_name = $request->has('last_name') && !is_null($request['last_name']) && $request['last_name'] != '' ? $request['last_name'] : $profile->last_name;
            $profile->gender = $request->has('gender') && !is_null($request['gender']) && $request['gender'] != '' ? $request['gender'] : $profile->gender;
            $profile->birthday = $request->has('birthday') && !is_null($request['birthday']) && $request['birthday'] != '' ? $request['birthday'] : $profile->birthday;
            $profile->address = $request->has('address') && !is_null($request['address']) && $request['address'] != '' ? $request['address'] : $profile->address;
            $profile->language = $request->has('language') && !is_null($request['language']) && $request['language'] != '' ? $request['language'] : $profile->language;
            $profile->calendar = $request->has('calendar') && !is_null($request['calendar']) && $request['calendar'] != '' ? $request['calendar'] : $profile->calendar;
            $profile->timezone = $request->has('timezone') && !is_null($request['timezone']) && $request['timezone'] != '' ? $request['timezone'] : $profile->timezone;
            $profile->save();
            DB::commit();
            $user->Profile->Translate();
            return response()->json([
